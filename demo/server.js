...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/download') {
      const filename = url.searchParams.get('filename') || 'download.txt';
      const options = getCreateOptions(Object.fromEntries(url.searchParams));
      const disposition = create(filename, options);
      const body = [
        'content-disposition test download',
        '',
        `filename: ${filename}`,
        `header: ${disposition}`,
      ].join('\n');
